	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body">
				...
				<input  id="gradeid"/>
				<label>年级：</label><input type="text" name="gradename1" placeholder="年级名称（*必填）" id="gradename1" class="form-control"/>
            	<br>
				<label>专业：</label><input type="text" name="specialtyname1" placeholder="专业名称（*必填）" id="specialtyname1" class="form-control" onchange="inspection1()"/>
            	<br>
...
<script>
	...
	function editInfo(specialtyId) { 
		...
		$.ajax({
			...
			success:function(data){
				$("#specialtyname1").val(data.specialtys[0].specialtyName);
				$("#gradename1").val(data.specialtys[0].grade.gradeName);
				$("#gradeid").val(data.specialtys[0].grade.id);
			},
			error:function(msg){
				console.log(msg)
			}
	...
	function inspection1(){
		...
		$.ajax({
			type:"post",
			url:"<%=request.getContextPath()%>/specialty/inspection.do",
			data:{"specialtyName":specialtyName,"gradeId":gradeId},
			dataType:"text",
			success:function(data){	
				if(data==1){
					$("#updateInfo").attr("disabled", true);
					$("#specialtyname1").focus().select();
					$("#message03").fadeIn();
				}
				else{
					$("#message03").hide();
	...
	function showInput(){
		//选取学院后，错误消息隐藏
		$("#message2").hide();
		$(".specialty-content").html("<input type='text' class='form-control' id='specialty' hidden='hidden' placeholder='专业' required onchange='inspection()'/>");
