	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body">
				...
				<label>方向：</label><input type="text" name="directionname1" placeholder="班级（*必填）" id="directionname1" class="form-control"/>
            	<br>
				<label>班级：</label><input type="text" name="clazzname1" placeholder="班级（*必填）" id="clazzname1" class="form-control" onchange="inspection1()"/>
            	<br>
...
<script>
	function editInfo(clazzId) { 
		...
		$.ajax({
			...
			success:function(data){
				$("#clazzname1").val(data.clazzs[0].className);
				$("#directionname1").val(data.clazzs[0].direction.directionName);
				$("#directionId").val(data.clazzs[0].direction.id);
			},
			error:function(msg){
				console.log(msg)
			}
	...
	function inspection1(){
		...
		$.ajax({
			type:"post",
			url:"<%=request.getContextPath()%>/clazz/inspection.do",
			data:{"className":className,"directionId":directionId},
			dataType:"text",
			success:function(data){	
				if(data==1){
					$("#updateInfo").attr("disabled", true);
					$("#clazzname1").focus().select();
					$("#message03").fadeIn();
				}
				else{
					$("#message03").hide();
	...
	function showInput(){
		//选取学院后，错误消息隐藏
		$("#message2").hide();
		$(".clazz-content").html("<input type='text' class='form-control' id='clazz' hidden='hidden' placeholder='班级' required onchange='inspection()'/>");
