	    		...
	    		<tr>
	    			...
	    			<td>
	    				<c:if test="${isNow == true }">
	    					<a href="#" data-toggle="modal" data-target="#import1" id="import">
		    					<c:choose>
	    							<c:when test="${items.form.reviewEvalForm == null }">
	    			...
	    			<td>
	    				<c:if test="${items.form.reviewEvalForm != null }">
	    					<a href="<%=request.getContextPath() %>/document/download.do?randName=${items.form.reviewEvalForm}&documentName=${items.no }_${items.name }_小组评阅表_${items.form.reviewEvalForm}">下载</a>
	    				</c:if>
	    			</td>
	    		</tr>
			</c:forEach>
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body">
				<form id="form" action="<%=request.getContextPath() %>/attach/submitMidReview.do" method="post" enctype="multipart/form-data">
...
$("#save").click(function(){
	...
	function(){
		...
		$.ajax({
			type:"post",
			url:"<%=request.getContextPath() %>/attach/submitMidReview.do",
			data:form,
			dataType:"json",
			processData:false,
            contentType:false,
