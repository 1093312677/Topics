    	...
    	<tr class="info">
	    	...
	    	<c:forEach items="${students}" var="it">
	    		<tr>
	    			...
	    			<td>
	    				<a href="<%=request.getContextPath() %>/student/viewStudentOne.do?filter=no&no=${it.no }&id=<c:out value="${it.id }"></c:out>"><c:out value="${it.name }"></c:out></a>
	    			</td>
