    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
            	<input type="text" id="collegeName" name="collegeName"  class="form-control" placeholder="学院名称" onchange="inspection()"/>
            	<div class="panel panel-success" style="margin-top:30px;width:50%;margin-left:25%" id="message1">
	 			   <div class="panel-body">
	 			   		该学院已存在！
	 			   </div>
	 			</div>
...
<script>
	function inspection(){
		...
		$.ajax({
			type:"post",
			url:"<%=request.getContextPath()%>/college/inspection.do",
			data:{"collegeName":collegeName},
			dataType:"text",
			success:function(data){	
				if(data==1){
					$("#addConfirm").attr("disabled", true);
					$("#collegeName").focus().select();
					$("#message1").fadeIn();
				}
				else{
					$("#message1").hide();
