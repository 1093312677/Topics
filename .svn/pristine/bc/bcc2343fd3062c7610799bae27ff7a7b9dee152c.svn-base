...
public class DepartmentController_guo {
	...
	public String inspection(String departmentName,HttpServletRequest request,HttpServletResponse response){
		try {
			response.getWriter().println(departmentService.inspection(departmentName));
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}finally {
