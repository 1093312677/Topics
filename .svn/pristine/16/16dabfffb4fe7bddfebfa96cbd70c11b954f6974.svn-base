    		...
    		<tr>
    			...
    			<td >
    				<c:choose>
						 ...
						 <c:otherwise>  
							<c:forEach items="${items.limitNumbers }" var="limit">
    							<button type="button"  class="btn btn-primary" onclick="getLimitId(${limit.id },'change')" data-toggle="modal" data-target="#myModal">修改人数</button>
    						</c:forEach>	
						 </c:otherwise> 
    				</c:choose>
    			</td>
